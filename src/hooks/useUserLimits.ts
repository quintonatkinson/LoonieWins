...
export function useUserLimits() {
  ...
  const weeklyUsed = useMemo(
    () =>
      normalizeWeeklyUsage(
        weeklyEntriesUsed,
        profile?.weekly_entries_reset_at ?? null
      ),
  ...
  return {
    balance,
    /** @deprecated use weeklyLimitReached — kept for older call sites */
    ...
    canEnterFree,
    /** Mid-tier default — prefer getEntryCost(contest) for prize-tiered spend */
