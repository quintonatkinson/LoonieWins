 ...
 * Returns { expiryDate: ISO string, is_estimated_expiry: true } when using fallback (posted_at + 30 days).
 */
function extractExpiryDate(
  ...
  try {
    ...
    // invalid date handling: fall through to fallback
  }

  const postedAt = postedAtIso ? new Date(postedAtIso) : now
  const fallback = new Date(postedAt)
  fallback.setDate(fallback.getDate() + 30)
  return { expiryDate: fallback.toISOString(), is_estimated_expiry: true }
}
