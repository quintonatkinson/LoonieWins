...
export function useUserLimits() {
  const { balance, lastDailyEntryAt, subscriptionTier, spendPointsForEntry, useFreeEntry } = useUserEarn()

  const userIsFree = subscriptionTier === 'free'
  const entriesUsedToday = useMemo(() => isToday(lastDailyEntryAt), [lastDailyEntryAt])
  const dailyLimitReached = userIsFree && entriesUsedToday
  const canEnterFree = userIsFree && !entriesUsedToday

  const spendForEntry = useMemo(() => () => spendPointsForEntry(ENTRY_COST_PTS), [spendPointsForEntry])
